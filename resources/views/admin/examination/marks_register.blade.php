...
<main class="app-main">
                    ...
                    <form method="GET" action="{{ url('admin/examination/marks_register') }}">
                        <div class="row g-3 align-items-end">
                            ...
                            <div class="col-md-4 d-flex gap-2">
                                ...
                                <a href="{{ url('admin/examination/marks_register') }}" class="btn btn-outline-secondary flex-fill">
    ...
    <div class="app-content">
        <div class="container-fluid">
            ...
            <form action="{{ url('admin/examination/submit_marks_register') }}"
                  method="post" id="SubmitMarksForm">
                @csrf
                                    ...
                                    <tr class="table-light text-uppercase text-secondary" style="font-size:.7rem;letter-spacing:.05em;">
                                        ...
                                        @foreach($getSubject as $subject)
                                            <th style="min-width:160px;" class="text-center">
                                                <div class="fw-semibold text-dark small">{{ $subject->subject_name }}</div>
                                                <div class="d-flex justify-content-center gap-1 mt-1 flex-wrap">
                                                    <span class="badge bg-secondary bg-opacity-10 text-secondary" style="font-size:.62rem;">
                                                        {{ $subject->subject_type == 0 ? 'Theory' : 'Practical' }}
                                                    </span>
                                                    <span class="badge bg-primary bg-opacity-10 text-primary" style="font-size:.62rem;">
                                                        Full: {{ $subject->full_mark }}
                                                    </span>
                                                    <span class="badge bg-warning bg-opacity-10 text-warning" style="font-size:.62rem;">
                                            ...
                                            <tr>
                                                ...
                                                @foreach($getSubject as $subject)
                                                    ...
                                                    <td class="align-top p-2">
                                                        ...
                                                        <div class="row g-1">
                                                            <div class="col-6">
                                                                <label class="form-label mb-0" style="font-size:.68rem;color:#666;">Class Work</label>
                                                            ...
                                                            <div class="col-6">
                                                                <label class="form-label mb-0" style="font-size:.68rem;color:#666;">Home Work</label>
                                                                <input type="number" min="0"
                                                                       class="form-control form-control-sm mark-input"
                                                            ...
                                                            <div class="col-6">
                                                                <label class="form-label mb-0" style="font-size:.68rem;color:#666;">Test Work</label>
                                                                <input type="number" min="0"
                                                                       class="form-control form-control-sm mark-input"
                                                            ...
                                                            <div class="col-6">
                                                                <label class="form-label mb-0" style="font-size:.68rem;color:#666;">Exam</label>
                                                                <input type="number" min="0"
                                                                       class="form-control form-control-sm mark-input"
                                                        ...
                                                        @if(!empty($totalMark))
                                                        <div class="mt-2 p-2 rounded-2 bg-light border" style="font-size:.72rem;">
                                                            <div><span class="text-muted">Total:</span> <strong>{{ $totalMark }}</strong> / {{ $subject->full_mark }}</div>
                                                            @if(!empty($getGrade))
                                                            <div><span class="text-muted">Grade:</span> <strong>{{ $getGrade }}</strong></div>
                                                            @endif
                                                            <div>
                                                ...
                                                <td class="align-top text-center p-3">
                                                    <button type="button" class="btn btn-sm btn-outline-success single-save-row-btn w-100 mb-3">
                                                        <i class="bi bi-floppy-fill me-1"></i>Save Row
                                                    </button>
                                                    @php
                                                        $percentage = $totalFullMark > 0 ? round($totalStudentMark * 100 / $totalFullMark) : 0;
                                                        $summaryGrade = App\Models\MarksGradeModel::getGrade($percentage);
                                                    @endphp
...
$(document).ready(function () {
    ...
    $('#SubmitMarksForm').on('submit', function (e) {
        ...
        $.ajax({
            ...
            success: function (data) {
                btn.prop('disabled', false).html('<i class="bi bi-floppy-fill me-2"></i>Submit All Marks');
    ...
    $(document).on('click', '.single-save-row-btn', function () {
        ...
        $.ajax({
            ...
            success: function (res) {
                btn.prop('disabled', false).html('<i class="bi bi-floppy-fill me-1"></i>Save Row');
    ...
    $(document).on('click', '.save-single-column-btn', function () {
        ...
        if (!colInputs.length) { alert('No data found for this column.'); return; }
        ...
        $.ajax({
            ...
            dataType: 'json',
            success: function (res) {
                btn.prop('disabled', false).html('<i class="bi bi-floppy me-1"></i>Save Col');
                showAjaxMessage(res.message, 'success');
            },
            error: function (xhr) {
                btn.prop('disabled', false).html('<i class="bi bi-floppy me-1"></i>Save Col');
