...
@section('style')
    <style>
        ...
        .fee-row input[type=date],
        .fee-row input[type=number] {
            min-width: 130px;
        }

        /* Disabled state for unchecked rows */
    ...
    <main class="app-main">
                ...
                <div class="row align-items-center py-1">
                    <div class="col-sm-6">
                        <h4 class="mb-0 fw-semibold">
                            <i class="bi bi-collection me-2 text-primary"></i>Add Fee Group
                        </h4>
                    </div>
        ...
        <div class="app-content">
            <div class="container-fluid">
                ...
                <form action="{{ url('admin/fee_group/add') }}" method="POST" id="fgForm">
                    @csrf
                            ...
                            <div class="row g-3">
                                <div class="col-md-6">
                                    ...
                                    <input type="text" name="name" class="form-control"
                                        placeholder="e.g. Class One Half Yearly" value="{{ old('name') }}" required>
                                </div>
                                <div class="col-md-3">
                                ...
                                <div class="col-12">
                                    ...
                                    <textarea name="description" class="form-control" rows="2" placeholder="Optional note about this group...">{{ old('description') }}</textarea>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="card border-0 shadow-sm rounded-3 mb-3">
                        <div class="card-header bg-transparent border-bottom d-flex align-items-center gap-2">
                            <span class="small fw-semibold text-muted text-uppercase">
                                <i class="bi bi-list-check me-1"></i> Select Fee Types &amp; Set Details
                            </span>
                        ...
                        <div class="card-body p-0">
                            <div class="table-responsive">
                                <table class="table table-bordered align-middle mb-0" id="feeTable">
                                    ...
                                    <tbody>
                                        @forelse($getFeeTypes as $ft)
                                            <tr class="fee-row disabled-row" id="row-{{ $ft->id }}"
                                                ...
                                                <td>
                                                    <input type="date" name="due_dates[{{ $ft->id }}]"
                                                        ...
                                                        value="{{ old('due_dates.' . $ft->id) }}" disabled>
                                                </td>
                                                <td>
                                                    <input type="number" name="amounts[{{ $ft->id }}]"
                                                        id="amt-{{ $ft->id }}" class="form-control form-control-sm"
                                                        step="0.01" min="0"
                                                        value="{{ old('amounts.' . $ft->id, $ft->amount) }}" disabled>
                                                </td>
                                            </tr>
                                        @empty
                                            <tr>
                                                <td colspan="4" class="text-center text-muted py-4">
                                                    No active fee types found.
                                                    <a href="{{ url('admin/fee_type/add') }}">Create one first.</a>
                                                </td>
                                            </tr>
                                        @endforelse
                                    </tbody>
                    ...
                    {{-- ── Summary bar ─────────────────────────────────────── --}}
                    <div class="card border-0 shadow-sm rounded-3 mb-3" id="summaryBox" style="display:none;">
                        <div class="card-body py-2 px-3 d-flex align-items-center gap-3 flex-wrap">
                            ...
                            <div class="ms-auto">
                                <button type="submit" class="btn btn-primary" id="saveBtn">
                                    <i class="bi bi-save me-1"></i> Save Fee Group
                                </button>
                            </div>
                        </div>
                    </div>

                    {{-- Fallback save button when nothing selected yet --}}
                    <div class="text-end" id="staticSave">
    ...
    <script>
        ...
        function updateSummary() {
            ...
            document.querySelectorAll('.fee-cb:checked').forEach(cb => {
                count++;
                const amt = parseFloat(document.getElementById('amt-' + cb.value).value) || 0;
                total += amt;
            });
            ...
            document.getElementById('summaryBox').style.display = count > 0 ? '' : 'none';
            document.getElementById('staticSave').style.display = count > 0 ? 'none' : '';
        }

        // Recalculate total when any amount changes
        document.querySelectorAll('[id^="amt-"]').forEach(inp => {
            inp.addEventListener('input', updateSummary);
        });

        // Form guard
        document.getElementById('fgForm').addEventListener('submit', function(e) {
            const checked = document.querySelectorAll('.fee-cb:checked').length;
            if (checked === 0) {
                e.preventDefault();
                alert('Please select at least one Fee Type.');
                return;
            }
            // Ensure disabled inputs send their values (re-enable before submit)
