                ...
                <div class="col-lg-7">
                    <div class="card border-0 shadow-sm rounded-3">
                        ...
                        <div class="card-body p-4">
                            <form action="{{ url('admin/library/issue/add') }}" method="POST">
                                @csrf
                                ...
                                <div class="mb-3">
                                    <label class="form-label fw-semibold">Issue To <span class="text-danger">*</span></label>
                                    <select name="member_id" class="form-select" required>
                                        ...
                                        <optgroup label="Students">
                                            @foreach($getMembers->where('user_type', 3) as $m)
                                                <option value="{{ $m->id }}" {{ old('member_id') == $m->id ? 'selected' : '' }}>
                                                    {{ $m->name }} {{ $m->last_name }}
                                                    @if($m->admission_number)({{ $m->admission_number }})@endif
                                                </option>
                                            @endforeach
                                        </optgroup>
                                    </select>
                                ...
                                <div class="d-flex gap-2">
                                    <a href="{{ url('admin/library/issue/list') }}" class="btn btn-outline-secondary flex-grow-1">Cancel</a>
